// client/src/components/BedAllocationBoard.tsx
import { useState, useCallback } from "react";
import { useRealtimeBeds } from "../hooks/useRealtimeBeds";
import { BedStatusChangedPayload } from "../services/socket.service";
...
type BedOccupancy = { vacant: number; occupied: number; cleaning: number; maintenance: number };
...
const BED_TYPE_ICON: Record<string, string> = {
  general: "🛏️", icu: "❤️‍🩹", emergency: "🚨",
  private: "🏠", pediatric: "🧸", maternity: "🤱",
};
...
export default function BedAllocationBoard({ departmentId, initialBeds = [] }: Props) {
  ...
  const [occupancy, setOccupancy] = useState<BedOccupancy>({ vacant: 0, occupied: 0, cleaning: 0, maintenance: 0 });
  ...
  // useRealtimeBeds handles all socket subscription + toasts internally
  useRealtimeBeds({ departmentId, onBedStatusChanged, showToasts: true });
    ...
    <div className="min-h-screen bg-gray-950 text-white p-6 font-sans">
        ...
        return (
          <section key={statusKey} className="mb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {group.map((bed) => <BedCard key={bed._id} bed={bed} />)}
            </div>
          </section>
        );
      })}
      ...
      {beds.length === 0 && (
        <p className="text-gray-600 text-sm text-center py-20">
          No beds loaded. Pass them via <code>initialBeds</code>.
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className={`rounded-xl p-4 border border-white/5 ${style.bg} flex flex-col gap-3`}>
      <div className="flex items-start justify-between">
        <div>
          <span className="text-lg font-bold">{BED_TYPE_ICON[bed.bedType]} {bed.bedNumber}</span>
          <p className="text-xs text-gray-400">{bed.ward} · Floor {bed.floor}</p>
        </div>
      ...
      {bed.status === "occupied" && bed.currentPatient?.user && (
        <div className="bg-black/20 rounded-lg px-3 py-2 text-sm">
          ...
          {bed.assignedAt && (
            <p className="text-gray-500 text-xs mt-1">Since {new Date(bed.assignedAt).toLocaleDateString()}</p>
          )}
          {bed.expectedDischargeDate && (
            <p className="text-gray-500 text-xs">Discharge: {new Date(bed.expectedDischargeDate).toLocaleDateString()}</p>
          )}
        </div>
      )}
