...
import { useQueue, QueuePayload, QueueEntry } from "../hooks/useQueue";
...
export default function LiveQueueBoard({ doctorId, departmentId }: Props) {
  ...
  const onQueueUpdated = useCallback((payload: QueuePayload) => {
    setQueue(payload.queue);
    setLastUpdated(new Date(payload.updatedAt).toLocaleTimeString());
  }, []);

  useQueue({ doctorId, departmentId, onQueueUpdated });
      ...
      {called.length > 0 && (
        <div className="mb-6">
          ...
          <div className="flex gap-3 flex-wrap">
            {called.map((entry) => (
              <div key={entry._id} className="bg-green-700 rounded-xl px-5 py-3 text-2xl font-bold">
                #{entry.tokenNumber}
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {waiting.map((entry) => (
              ...
              >
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-semibold ${PRIORITY_COLOR[entry.priority]}`}
                >
