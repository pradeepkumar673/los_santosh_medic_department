...
export const initSocket = (httpServer: HttpServer): SocketIOServer => {
  ...
  io.on("connection", (socket: AuthenticatedSocket) => {
    ...
    if (socket.user) {
      ...
      // Auto-join emergency command room for relevant roles
      if (["admin", "nurse", "doctor", "reception"].includes(socket.user.role)) {
        socket.join(ROOMS.emergencyCommand);
      }
    }
    ...
    socket.on("join:emergency-command", () => {
      if (["admin", "nurse", "doctor", "reception"].includes(socket.user?.role || "")) {
        socket.join(ROOMS.emergencyCommand);
      }
    });
